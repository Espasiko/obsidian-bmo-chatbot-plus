...
export class BMOView extends ItemView {
    ...
    async onOpen(): Promise<void> {
        ...
        // FIX UX: al abrir siempre arrancar conversación NUEVA (vacía). La última
        // queda accesible desde la barra lateral o el botón "Continuar última".
        await startNewActiveConversation(this.plugin, messageHistory);
        ...
        createConversationsSidebar(this.plugin, chatbotContainer, async () => {
            ...
            // Tras Nuevo chat / borrar la conversacion activa el chat queda vacio y el bug
            // nativo de teclado reaparece: aplicar la patada de oclusion solo en ese caso.
            if (messageHistory.length === 0) {
                this.kickKeyboardOcclusion('switch-empty');
            }
        });
        ...
        // Force focus on textarea after Obsidian finishes layout.
        // FIX 07/06/2026 (CONFIRMADO): el bug es NATIVO de Electron — el teclado no se enruta
        // al textarea de la barra lateral hasta una OCLUSION de ventana (minimizar->restaurar).
        // El resize NO lo arregla; minimizar SI (confirmado por el usuario). Por eso aqui, cuando
        // el chat esta VACIO (unico caso roto: conversacion nueva / tras borrar la activa), se
        // dispara `kickKeyboardOcclusion()`. Si hay mensajes, el teclado ya funciona y no se toca.
        const focusTextarea = () => {
            if (this.textareaElement && document.body.contains(this.textareaElement)) {
        ...
        this.app.workspace.onLayoutReady(() => {
            ...
            if (messageHistory.length === 0) {
                setTimeout(() => this.kickKeyboardOcclusion('open-empty'), 200);
            }
        });

        // Also re-focus when user clicks anywhere in the chatbox area
        chatbox.addEventListener('click', (e) => {
            const target = e.target as HTMLElement;
            if (target !== submitButton && !target.closest('.submit-button')) {
                focusTextarea();
            }
        });
    }
    ...
    // Acceso seguro a la ventana Electron (BrowserWindow) desde el renderer de Obsidian.
    getElectronWindowSafe(): any {
        try {
    ...
    // Arreglo del bug nativo de teclado: una OCLUSION de ventana (minimizar->restaurar) re-enruta
    // el teclado al textarea. Es lo unico que funciona (el resize no). Se llama solo cuando el
    // chat esta vacio. restore() preserva el estado maximizado previo en Windows.
    kickKeyboardOcclusion(_reason: string): void {
        const focusTa = () => {
            if (this.textareaElement && document.body.contains(this.textareaElement)) {
                this.textareaElement.focus();
            }
        };
        const win = this.getElectronWindowSafe();
        if (!win || typeof win.minimize !== 'function') { focusTa(); return; }
        try {
            win.minimize();
            setTimeout(() => {
...
export function renderMessageContainer(plugin: BMOGPT, settings: BMOSettings): void {
    ...
    if (messageHistory.length === 0) {
        // Pantalla de bienvenida para conversación nueva
        const welcomeWrapper = document.createElement('div');
        welcomeWrapper.className = 'bmo-welcome';
        welcomeWrapper.style.cssText = 'display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.5rem;padding:2rem;text-align:center;height:100%;';

        const greeting = document.createElement('div');
        greeting.className = 'botMessage';
        greeting.style.cssText = 'padding:1rem;border-radius:8px;max-width:80%;margin:0;';
        greeting.textContent = settings.general.welcomeMessage || '¡Hola! ¿En qué puedo ayudarte hoy?';
        welcomeWrapper.appendChild(greeting);

        // Botón "Continuar última conversación"
        const continueBtn = document.createElement('button');
        continueBtn.textContent = 'Continuar última conversación';
        continueBtn.className = 'bmo-continue-btn';
        continueBtn.style.cssText = 'padding:0.5rem 1rem;border-radius:4px;cursor:pointer;background:var(--interactive-accent);color:var(--text-on-accent);border:none;font-size:0.9rem;';
        continueBtn.addEventListener('click', async () => {
            const profile = settings.profiles.profile;
            const convs = await listConversations(plugin, profile);
            if (convs.length > 0 && convs[0].filePath) {
                await switchActiveConversation(plugin, convs[0].filePath, messageHistory);
                renderMessageContainer(plugin, settings);
                refreshConversationHeader();
            }
        });
        welcomeWrapper.appendChild(continueBtn);

        messageContainer.appendChild(welcomeWrapper);
    } else {
